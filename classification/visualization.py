...
pour fonctionner avec n'importe quel dataset.
"""
...
def print_comparison_table(
    ...
    for model_name, data in results.items():
        ...
        for m in ["AUC", "F1", "Accuracy", "Kappa"]:
...
def analyze_convergence(
    ...
    """
    N* = premier N_i où le gain relatif d'AUC vers N_{i+1} < threshold.
    AUC : higher is better → gain = (mean[i+1] - mean[i]) / mean[i].
    """
    print(f"\n{'=' * 70}")
    ...
    for model_name, data in results.items():
        ...
        for i in range(len(sizes) - 1):
            rel_gain = (mean[i + 1] - mean[i]) / (abs(mean[i]) + 1e-10)
